...
def pattern_name
  name = @browser.find_element(:id, 'vmMainPage').find_element(:tag_name, 'h1').text
end
...
def pattern_collection
  ...
  if !@browser.find_elements(:id, 'brand_image').empty?
    ...
    if rs.entries.size == 0
      @con.exec("INSERT INTO pattern_collection(name, pattern_company_id) VALUES('#{pattern_collection_name}', 4)")
...
def pattern_description
  if !@browser.find_element(:css, '#vmMainPage table:nth-child(2) tr:nth-child(2) img+p').size() != 0
    description = @browser.find_element(:css, '#vmMainPage table:nth-child(2) tr:nth-child(2) img+p').text
  else
    description = @browser.find_element(:css, '#vmMainPage table:nth-child(2) tr:nth-child(2) table tr td:nth-child(2) p:first-child').text
...
def images 
  ...
  #images_list
  #
  @browser.find_element(:css, '#vmMainPage table:nth-child(2) tr:nth-child(2) img').attribute('src')
end
...
def go_to_search_result_url(start)
  ...
  @browser.get "http://www.stylearc.com.au/stylearc/index.php?option=com_virtuemart&page=shop.browse&category_id=6&Itemid=47"
  # http://www.stylearc.com.au/stylearc/index.php?option=com_virtuemart&page=shop.browse&category_id=5&Itemid=47
  # http://www.stylearc.com.au/stylearc/index.php?option=com_virtuemart&page=shop.browse&category_id=7&Itemid=47
  # http://www.stylearc.com.au/stylearc/index.php?option=com_virtuemart&page=shop.browse&category_id=2&Itemid=47
  # http://www.stylearc.com.au/stylearc/index.php?option=com_virtuemart&page=shop.browse&category_id=6&Itemid=47
end
...
urls = []
@browser.find_elements(:css, '#vmMainPage table table td a').each do | result |
...
patterns = {:pattern_name => pattern_name, :pattern_collection => pattern_collection, :pattern_description => pattern_description, :images => images, :line_art => line_art, :url => pattern_page_url}
...
query = "INSERT INTO patterns(pattern_company_id, 
                              ...
                              line_drawing,
                              description,
                              url) VALUES('4','#{patterns[:pattern_name]}',#{patterns[:pattern_collection]},'#{patterns[:images]}','#{patterns[:line_art]}','#{patterns[:pattern_description]}', '#{patterns[:url]}')"
puts query
