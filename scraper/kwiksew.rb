...
def pattern_collection
  ...
  if !@browser.find_elements(:id, 'brand_image').empty?
    ...
    if rs.entries.size == 0
      @con.exec("INSERT INTO pattern_collection(name, pattern_company_id) VALUES('#{pattern_collection_name}', 2)")
...
def pattern_description
  description = @browser.find_element(:css, '#product_short_description p:first-child').text
...
def go_to_search_result_url(start)
  ...
  @browser.get "http://kwiksew.mccall.com/tops-blouses-pages-3038.php"
end
...
patterns = {:pattern_number => pattern_number, :pattern_collection => pattern_collection, :pattern_description => pattern_description, :pattern_for => pattern_for, :garment_type => garment_type, :images => images, :line_art => line_art, :url => pattern_page_url}
...
query = "INSERT INTO patterns(pattern_company_id, 
                              ...
                              line_drawing,
                              description,
                              url) VALUES('7','#{patterns[:pattern_number]}',#{patterns[:pattern_collection]},'#{patterns[:images]}','#{patterns[:line_art]}','#{patterns[:pattern_description]}', '#{patterns[:url]}')"
puts query
