...
def go_to_search_result_url(start)
  ...
  @browser.get "http://mccallpattern.mccall.com/new-sewing-patterns-pages-96.php"
end
...
query = "INSERT INTO patterns(pattern_company_id, 
                              ...
                              line_drawing,
                              description,
                              url) VALUES('2','#{patterns[:pattern_number]}',#{patterns[:pattern_collection]},'#{patterns[:images]}','#{patterns[:line_art]}','#{patterns[:pattern_description]}', '#{patterns[:url]}')"
puts query
