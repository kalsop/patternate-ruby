...
def pattern_name
  name = @browser.find_element(:class, 'product_information').find_element(:tag_name, 'h1').text
end
...
def pattern_collection
  ...
  if !@browser.find_elements(:id, 'brand_image').empty?
    ...
    if rs.entries.size == 0
      @con.exec("INSERT INTO pattern_collection(name, pattern_company_id) VALUES('#{pattern_collection_name}', 5)")
...
def pattern_description
  description = @browser.find_element(:css, '.description').text
...
def images 
  ...
  #images_list
  #
  @browser.find_element(:css, '#main_photo a.top img').attribute('src')
end

def line_art
  @browser.find_element(:css, '.thumbnails .thumbnail:last-of-type a').attribute('href')
  # find_line_art = @browser.find_element(:css, '.thumbnails .thumbnail:last a').attribute('href')
  # find_line_art.each do |line_art_img|
  #   # if line_art_image.attribute('href').include? 'cover-med'
  #   #   line_art_image.attribute('href')
  #   # end
  # end
end
...
def go_to_search_result_url(start)
  ...
  @browser.get "https://www.colettepatterns.com/sewing/outerwear"
  # https://www.colettepatterns.com/sewing/separates
  # https://www.colettepatterns.com/sewing/dresses
  # https://www.colettepatterns.com/sewing/outerwear
end
...
urls = []
@browser.find_elements(:css, 'h4.name a').each do | result |
...
query = "INSERT INTO patterns(pattern_company_id, 
                              ...
                              line_drawing,
                              description,
                              url) VALUES('5','#{patterns[:pattern_name]}',#{patterns[:pattern_collection]},'#{patterns[:images]}','#{patterns[:line_art]}','#{patterns[:pattern_description]}', '#{patterns[:url]}')"
puts query
